    ...
    <main className="flex min-h-screen items-center justify-center bg-neutral-950 p-4">
      <div className="w-full max-w-md">
        <div className="mb-6 flex flex-col items-center text-center">
          ...
          <h1 className="mt-4 text-[22px] font-extrabold tracking-tight text-white">
            Creator Agency
          </h1>
        ...
        <div className="rounded-2xl border border-neutral-800/80 bg-neutral-900/60 p-7 shadow-2xl">
          <div className="rounded-xl border border-violet-500/20 bg-violet-500/5 p-3 text-center">
            <div className="text-[10.5px] font-bold uppercase tracking-wider text-violet-300">
              Invitation de
            </div>
            <div className="mt-0.5 text-[13px] font-bold text-white">
              {invitation.inviter?.full_name ?? invitation.inviter?.email}
            </div>
            <div className="mt-2 inline-flex items-center rounded-md bg-amber-500/10 px-2 py-0.5 text-[10.5px] font-bold uppercase tracking-wider text-amber-300 ring-1 ring-inset ring-amber-500/20">
              Rôle : {invitation.role}
            </div>
          </div>

          <h2 className="mt-5 text-[14px] font-bold text-white">
            Crée ton compte
          </h2>
...
function ErrorScreen({
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-neutral-950 p-4">
      <div className="w-full max-w-md rounded-2xl border border-neutral-800/80 bg-neutral-900/60 p-8 text-center shadow-2xl">
        <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-2xl bg-rose-500/10 text-rose-300 ring-1 ring-inset ring-rose-500/20">
          <AlertTriangle className="h-5 w-5" />
        </div>
        <h2 className="mt-4 text-[18px] font-extrabold text-white">{title}</h2>
        <p className="mt-2 text-[13px] text-neutral-400">{message}</p>
        {actionHref && actionLabel && (
          <Link
            href={actionHref}
            className="mt-5 inline-flex h-10 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-fuchsia-500 px-4 text-[13px] font-bold text-white hover:brightness-110"
          >
            {actionLabel}
          </Link>
        )}
